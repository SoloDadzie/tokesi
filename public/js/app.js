...
if (menuBtn && mobileMenu) {
  menuBtn.addEventListener('click', () => {
    ...
    const menuSvg = menuBtn.querySelector('.menu-svg');
    const closeSvg = menuBtn.querySelector('.close-svg');
    if (menuSvg) menuSvg.style.display = isOpen ? 'block' : 'none';
    if (closeSvg) closeSvg.style.display = isOpen ? 'none' : 'block';
  });
}
...
function isCartOpen() {
  return cartPanel && cartPanel.style.right === '0px';
}
